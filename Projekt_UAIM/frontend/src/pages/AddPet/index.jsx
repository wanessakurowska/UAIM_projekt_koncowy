...
const AddPet = () => {
  ...
  const [filteredRasy, setFilteredRasy] = useState([]); 
  const [success, setSuccess] = useState(false);
  const [error, setError] = useState("");
  const navigate = useNavigate();

  // Pobranie listy gatunków i ras
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [gatunkiResponse, rasyResponse] = await Promise.all([
          apiClient.get("/api/species"),
          apiClient.get("/api/races"),
        ]);
  ...
  useEffect(() => {
    // Filtrowanie ras na podstawie wybranego gatunku
    setFilteredRasy(
