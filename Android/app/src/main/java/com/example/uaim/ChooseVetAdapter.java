...
public class ChooseVetAdapter extends BaseAdapter {

    private Context context;
    private JSONArray vets;

    public ChooseVetAdapter(Context context, JSONArray vets) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            ...
            convertView = inflater.inflate(R.layout.single_row_ww, parent, false);
        }
        ...
        Button buttonWW = convertView.findViewById(R.id.buttonWW);
        ...
        try {
            ...
            textViewStatus.setText("Wykształcenie: " + vet.getString("wyksztalcenie"));
        } catch (Exception e) {
            e.printStackTrace();
        }


        buttonWW.setOnClickListener(v -> {

            Intent intent = new Intent(context, CalendarActivity.class);
            try {
                JSONObject vet = vets.getJSONObject(position);
                int vetId = vet.getInt("id_weterynarza");
                intent.putExtra("vetID", vetId);
                intent.putExtra("vetSurname", vet.getString("nazwisko"));
            } catch (Exception e) {
                e.printStackTrace();
            }
